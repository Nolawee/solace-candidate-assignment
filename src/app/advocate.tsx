...
export default function AdvocateCard({ advocate }: AdvocateCardProps) {
  return (
    <article className="grid grid-cols-[180px_1fr_300px] gap-8 p-8 border border-gray-300 rounded-lg bg-white shadow-sm items-center lg:grid-cols-[180px_1fr_300px] max-lg:flex max-lg:flex-col max-lg:items-start max-lg:gap-0">
      ...
      <div className="flex flex-col gap-2 max-lg:overflow-hidden">
        ...
        <div className="flex gap-6 mb-2 max-lg:flex-wrap max-lg:mt-2">
          <div className="flex items-center gap-2 text-[#666] text-sm">
            <svg aria-hidden="true" className="w-5 h-5" data-block="true" fill="#666" width="24" height="24" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-2 text-[#666] text-sm">
            <svg aria-hidden="true" className="w-5 h-5" width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex items-center gap-2 text-[#666] text-[1.1em] mt-3">
          <svg aria-hidden="true" className="w-5 h-5" data-block="true" fill="#666" width="24" height="24" viewBox="0 0 24 24">
        ...
        <div className="inline leading-[1.8]">
          {advocate.specialties.map((specialty, sIndex) => (
            <span key={sIndex}>
              <span className="text-[#3f937c] italic underline underline-offset-2">{specialty}</span>
      ...
      <div className="flex flex-col gap-3 items-end justify-center max-lg:clear-both max-lg:items-stretch max-lg:flex-row max-lg:justify-start max-lg:gap-4 max-lg:mt-6">
        <button 
          className="bg-[#347866] text-white w-[250px] h-[50px] text-lg font-medium rounded-[10px] hover:bg-[#3f937c] max-lg:flex-[0_0_auto]" 
          onClick={() => window.location.href = `tel:+1${advocate.phoneNumber}`}
        >
